...
class SosScreenUI extends StatelessWidget {
  ...
  final bool loading;
  final String search;
  final String? selectedStatus, selectedSeverity;
  final List<String> statusTabs, severityOptions;
  ...
  const SosScreenUI({
    ...
    this.statusTabs = const ['All', 'OPEN', 'RESOLVED', 'CLOSED'],
  ...
  Widget _header(bool hasFilters) {
    final open = sosList
        .where((s) => s['sos_status'] == 'OPEN')
        .length;
    final resolved = sosList
        .where((s) => s['sos_status'] == 'RESOLVED')
        .length;
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: _stat(
                  'Total SOS', '${sosList.length}',
                  Icons.warning_rounded, FisheriesOfficerOcean.red,
                  filterValue: null,
                ),
              ),
              ...
              Expanded(
                child: _stat(
                  'Open', '$open',
                  Icons.error_outline_rounded, FisheriesOfficerOcean.orange,
                  filterValue: 'OPEN',
                ),
              ),
              const SizedBox(width: 8),
          ...
          FisheriesOfficerOceanSearchField(
            ...
            hintText: 'Search by boat / reference...',
  ...
  Widget _stat(String title, String value, IconData icon, Color color,
    ...
    return GestureDetector(
      onTap: onStatusCardTap != null ? () => onStatusCardTap!(filterValue) : null,
  ...
  Widget _list(BuildContext context) => ListView.builder(
    ...
    itemBuilder: (_, i) => _card(context, sosList[i]),
  );

  Widget _card(BuildContext context, dynamic s) {
    final ref = s['sos_ref_no'] ?? '—';
    final vref = s['reference_no'];
    final boatName = s['boat_name'] ?? '—';
    final reg = s['boat_reg_no'];
    final type = s['sos_type'] ?? '—';
    final severity = (s['severity'] ?? 'LOW').toString().toUpperCase();
    final status = s['sos_status'] ?? 'OPEN';
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: const [
            BoxShadow(color: Color(0x0F0B2540), blurRadius: 6, offset: Offset(0, 2)),
          ],
        ),
        child: ClipRRect(
          borderRadius: BorderRadius.circular(14),
          child: Container(
            decoration: const BoxDecoration(
              ...
              border: Border(
                ...
                right: BorderSide(color: FisheriesOfficerOcean.border),
                bottom: BorderSide(color: FisheriesOfficerOcean.border),
              ),
            ),
            child: Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                Container(
                  width: 4,
                  height: 120, // Approximate max content height to fill the area down
                  color: bc,
                ),
                // ── Card content ──────────────────────────────────
                Expanded(
                          ...
                          children: [
                            ...
                            GestureDetector(
                              ...
                              child: Container(
                                width: 38, height: 38,
                                decoration: BoxDecoration(
                                  color: FisheriesOfficerOcean.primary.withValues(alpha: .12),
                                  borderRadius: BorderRadius.circular(10),
                                  border: Border.all(
                        ...
                        Row(
                          children: [
                            ...
                            Expanded(
                              child: Align(
                                ...
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: bc.withValues(alpha: .10),
                                    borderRadius: BorderRadius.circular(20),
                                    border: Border.all(color: bc.withValues(alpha: .20)),
                                  ),
                                  child: Row(
                                    mainAxisSize: MainAxisSize.min,
                                    children: [
                                      ...
                                      Flexible(
                                        child: Text(
                                          ...
                                          maxLines: 1, overflow: TextOverflow.ellipsis,
                                          style: TextStyle(
                                            color: bc, fontSize: 9.5, fontWeight: FontWeight.w800,
                                          ),
                                        ),
                                      ),
                                    ],
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                color: sevColor.withValues(alpha: .14),
                                borderRadius: BorderRadius.circular(20),
                                border: Border.all(color: sevColor.withValues(alpha: .28)),
                              ),
                              child: Text(severity,
                                  maxLines: 1, overflow: TextOverflow.ellipsis,
                                  style: TextStyle(
  ...
  // DETAIL SHEET
  // ─────────────────────────────────────────────────────────────
  void _showDetails(BuildContext context, dynamic s) {
          ...
          children: [
            ...
            Expanded(
              child: ListView(
                ...
                children: [
                  _detailRow('SOS Reference', s['sos_ref_no']),
                  _detailRow('Voyage Reference', s['reference_no']),
                  _detailRow('Boat', s['boat_name']),
                  _detailRow('Boat Reg. No.', s['boat_reg_no']),
                  _detailRow('Raised By', s['raised_by_name']),
                  _detailRow('Type', s['sos_type']),
                  _detailRow('Severity', s['severity']),
                  _detailRow('Status', s['sos_status']),
                  _detailRow('Date & Time', _fmtFull(s['sos_datetime'])),
                  _detailRow('Latitude', s['latitude']),
                  _detailRow('Longitude', s['longitude']),
                  _detailRow('Location Source', s['location_source']),
                  _detailRow('Description', s['description']),
                  _detailRow('Acknowledged By', s['acknowledged_by_name']),
                  _detailRow('Acknowledged At',
                      _fmtFull(s['acknowledged_at'])),
                  _detailRow('Resolved At', _fmtFull(s['resolved_at'])),
                  _detailRow('Resolution Remarks', s['resolution_remarks']),
                  _detailRow(
                      'Response Time',
