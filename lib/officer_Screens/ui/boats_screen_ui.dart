...
class BoatsScreenUI extends StatelessWidget {
  final List<dynamic> boatList;
  final int total, currentPage, limit;
  final bool loading;
  final String search;
  final String? selectedStatus;
  final List<String> statusTabs;
  final int totalBoats, activeBoats, inactiveBoats, suspendedBoats;
  ...
  const BoatsScreenUI({
    ...
    this.totalBoats = 0,
    this.activeBoats = 0,
    this.inactiveBoats = 0,
    this.suspendedBoats = 0,
      ...
      child: Column(
        ...
        children: [
          ...
          FisheriesOfficerOceanSearchField(
            ...
            hintText: 'Search by boat name / reg no...',
  ...
  Widget _list(BuildContext context) => ListView.builder(
    ...
    itemBuilder: (_, i) => _card(context, boatList[i]),
  );

  Widget _card(BuildContext context, dynamic b) {
    final name = b['boat_name'] ?? '—';
    final reg = b['boat_reg_no'] ?? b['registration_no'];
    final owner = b['owner_name'];
    final type = b['boat_type'] ?? '—';
    final status = (b['boat_status'] ?? 'ACTIVE').toString().toUpperCase();
    final length = b['length_m'];
    final capacity = b['capacity_kg'];
    final registered = _fmt(b['registered_at']);
            ...
            Row(
              children: [
                if (length != null) ...[
                  ...
                  const SizedBox(width: 4),
                  Text('${(length as num).toStringAsFixed(1)} m',
                      style: const TextStyle(
                ...
                if (capacity != null) ...[
                  ...
                  const SizedBox(width: 4),
                  Text('${(capacity as num).toStringAsFixed(0)} kg',
                      style: const TextStyle(
                        color: FisheriesOfficerOcean.green, fontSize: 10,
                      )),
                  const SizedBox(width: 10),
                ],
                const Spacer(),
  ...
  // DETAIL SHEET
  // ─────────────────────────────────────────────────────────────
  void _showDetails(BuildContext context, dynamic b) {
          ...
          children: [
            ...
            Expanded(
              child: ListView(
                ...
                children: [
                  _detailRow('Boat Name', b['boat_name']),
                  _detailRow('Registration No.', b['boat_reg_no']),
                  _detailRow('Owner', b['owner_name']),
                  _detailRow('Owner Contact', b['owner_phone']),
                  _detailRow('Boat Type', b['boat_type']),
                  _detailRow('Status', b['boat_status']),
                  _detailRow('Length (m)', b['length_m']),
                  _detailRow('Capacity (kg)', b['capacity_kg']),
                  _detailRow('Engine No.', b['engine_no']),
                  _detailRow('Hull Material', b['hull_material']),
                  _detailRow('Home Port', b['home_port']),
                  _detailRow('License No.', b['license_no']),
                  _detailRow('License Expiry', _fmtFull(b['license_expiry'])),
                  _detailRow('Registered At', _fmtFull(b['registered_at'])),
                  _detailRow('Remarks', b['remarks']),
