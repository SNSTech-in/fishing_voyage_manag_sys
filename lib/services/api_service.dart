...
class ApiService {
  ...
  Future<Map<String, dynamic>> createProfile({
    ...
    print('📤 [createProfile] REQUEST: $body');
    
    final response = await _client.post(
      Uri.parse(ApiConstants.createProfile),
      headers: ApiConstants.authHeaders(setupToken),
      body: jsonEncode(body),
    ).timeout(const Duration(seconds: 30));
    ...
    if (data['success'] == true && data['data']?['access_token'] != null) {
      ...
      final session = await _db.getUserSession();
      if (session != null) {
        print('🔑 createProfile: Saving access_token');
  ...
  Future<Map<String, dynamic>> getPorts() async {
    ...
    final response = await _client.get(Uri.parse(ApiConstants.ports), headers: headers).timeout(const Duration(seconds: 30));
  ...
  Future<Map<String, dynamic>> addCrew({
    ...
    final headers = await _getHeadersWithAccessToken();
    print('📤 [addCrew] REQUEST: $crewData');
    
    final response = await _client.post(
      Uri.parse(ApiConstants.ownerCrew),
      headers: headers,
      body: jsonEncode(crewData),
    ).timeout(const Duration(seconds: 30));
    
    final data = jsonDecode(response.body);
    print('📥 [addCrew] RESPONSE: $data');
  ...
  Future<Map<String, dynamic>> endTrip({
    ...
    String? token, // Support optional token from parameter
  }) async {
